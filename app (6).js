  ...
  return `
    <div class="page">
      <main class="container" style="padding-top:3rem;padding-bottom:3rem">
        ...
        ${
          regs.length === 0
            ? `<div class="empty-state mt-6">No teams yet. Register first to appear here.</div>`
            : `<div class="team-grid mt-6" style="grid-template-columns:1fr">
                ${regs
                  .map(
                    (t, i) => `
                  <div style="display:flex;align-items:center;gap:1rem;padding:1rem;border:1px solid var(--line);border-radius:0.75rem;background:var(--raised)">
                    <span class="text-gold" style="font-family:var(--font-display);font-weight:700;min-width:2.5rem">#${String(i + 1).padStart(2, "0")}</span>
                    <div>
                      <div style="font-weight:700">${escapeHtml(t.teamName)}</div>
                      <div class="text-muted" style="font-size:0.8rem">${escapeHtml(MODE_LABELS[t.mode] || t.mode)} · ${(t.captain && t.captain.name) || "—"}</div>
                    </div>
                  </div>`
                  )
                  .join("")}
