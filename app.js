...
async function updateRevealPublic(val) {
  await db.collection(SETTINGS_COL).doc(SETTINGS_ID).set({ revealPublic: !!val }, { merge: true });
}

async function updateVsPublic(val) {
  await db.collection(SETTINGS_COL).doc(SETTINGS_ID).set({ vsPublic: !!val }, { merge: true });
}
...
function bindEvents() {
  ...
  if (tvp) {
    tvp.addEventListener("click", async () => {
      await updateVsPublic(!state.settings.vsPublic);
    });
  }
