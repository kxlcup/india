...
const state = {
  registrations: [],
  settings: { closed: false, rooms: { ...EMPTY_ROOMS } },
  ready: false,
  error: null,
  route: "/",
  // reveal
  ...
  return db
    ...
    .onSnapshot(
      (snap) => {
        ...
        state.adminRooms = JSON.parse(JSON.stringify(state.settings.rooms));
        state.ready = true;
        render();
      },
      (err) => {
...
async function renderVsPoster(a, b) {
  ...
  // Team A
  drawLogo(ctx, imgA, a.teamName, W / 2, 400, 260);
  ctx.fillStyle = "#F2EDE6";
  ctx.font = "700 40px Rajdhani, sans-serif";
  ctx.fillText((a.teamName || "").toUpperCase(), W / 2, 580);
  ctx.fillStyle = "#FFB627";
  ctx.font = "600 18px Inter, sans-serif";
  ctx.fillText(((a.tagline || MODE_LABELS[a.mode] || "CLASH SQUAD")).toUpperCase(), W / 2, 612);
  ...
  ctx.fillText("Stream live · Room ID on YouTube", W / 2, 1325);
      ...
      <section class="section">
        <div class="container-wide">
          <div class="flex flex-wrap items-center justify-between mb-4">
            ...
            <a href="#/reveal" class="btn-ghost" style="border-color:rgba(255,182,39,0.4);color:var(--gold)">Open reveal stage →</a>
          </div>
          ${
            regs.length === 0
...
function renderVs() {
  ...
  return `
    <div class="vs-page">
      ...
      <p class="text-muted" style="margin-top:0.5rem;font-size:0.875rem">Combine two squad logos into a shareable matchup poster for stream and WhatsApp status. New ember & gold theme.</p>
      ${
        regs.length < 2
  ...
  return `
    <div class="container-wide" style="padding:2.5rem 1rem">
      ...
      <section class="admin-section">
        ...
        <p class="text-muted" style="font-size:0.875rem;margin-bottom:1rem">Control the public reveal stage. Open it for stream / share the public Reveal page.</p>
        <div class="flex flex-wrap gap-2">
          <a href="#/reveal" class="btn-primary" style="min-height:auto;padding:0.5rem 1rem;font-size:0.875rem">Open Reveal stage →</a>
          <button class="btn-ghost" id="adminRevealPlay" style="min-height:auto;padding:0.5rem 1rem;font-size:0.875rem">
            ${state.revealPlaying ? "Pause auto-play" : "Start auto-play on Reveal"}
          </button>
        </div>
        ${
          regs.length
            ? `<p class="text-muted mt-2" style="font-size:0.8rem">${regs.length} teams · current index ${state.revealIdx + 1}</p>
               <div style="margin-top:1rem;max-width:20rem">${teamCardHtml(regs[state.revealIdx] || regs[0], state.revealIdx, true)}</div>`
      ...
      <section class="admin-section">
        ...
        <p class="text-muted" style="font-size:0.875rem;margin-bottom:1rem">Generate matchup posters from admin. Same tool is also available publicly on VS page.</p>
        ${
          regs.length < 2
...
function render() {
  ...
  let html = "";
  if (state.route === "/reveal") html = renderReveal();
  else if (state.route === "/vs") html = renderVs();
  else if (state.route === "/hall") html = renderHall();
  else if (state.route === "/admin") html = renderAdmin();
...
function bindEvents() {
  ...
  if (prev) prev.addEventListener("click", () => {
    ...
    setTimeout(() => {
      ...
      if (n) state.revealIdx = (state.revealIdx - 1 + n) % n;
  ...
  if (next) next.addEventListener("click", () => {
    ...
    setTimeout(() => {
      const n = state.registrations.length;
      if (n) state.revealIdx = (state.revealIdx + 1) % n;
      state.revealCurtain = false;
      render();
    }, 280);
  });
  if (play) play.addEventListener("click", () => {
    state.revealPlaying = !state.revealPlaying;
    if (state.revealTimer) { clearInterval(state.revealTimer); state.revealTimer = null; }
    if (state.revealPlaying) {
  ...
  const avd = document.getElementById("adminVsDownload");
  if (avd) avd.addEventListener("click", () => downloadVs());
}
