...
function renderVs() {
  ...
  if (!isAdmin && publicOk) return renderVsFullscreen();
...
function renderVsFullscreen() {
  ...
  if (pp.failedKey !== ppKey) {
    ...
    return `
      <div style="position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;background:#0a0806;overflow:hidden">
        ${
          pp.url
            ? `<img src="${pp.url}" alt="VS poster" style="max-width:100vw;max-height:100vh;max-height:100dvh;width:auto;height:auto;object-fit:contain;display:block" />`
