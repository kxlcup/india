...
function renderVs() {
  ...
  // Public visitors never get the generator — only the published poster
  if (!isAdmin && publicOk) return renderVsFullscreen(true);
  ...
  return `
    <div class="container-wide" style="padding:2.5rem 1rem">
      ...
      <section class="admin-section">
        ...
        <p class="text-muted" style="font-size:0.875rem;margin-bottom:1rem">
          Generate posters anytime here. Once published, the poster replaces the public main page and shows in the VS section.
        </p>
        <div class="flex flex-wrap gap-2" style="margin-bottom:1rem">
          ...
          ${
            ...
            (() => {
              ...
              return m.aId !== state.vsA || m.bId !== state.vsB || m.date !== (state.vsDate || "") || m.time !== (state.vsTime || "");
            })()
              ? `<button class="btn-ghost" id="updateVsLive" style="min-height:auto;padding:0.5rem 1rem;font-size:0.875rem">↻ Update live poster</button>`
              : ""
          }
        ...
        <p style="font-size:0.8rem;margin-bottom:1rem">
          Status:
          ${state.settings.vsPublic
            ? '<span class="text-success">Poster is live on main page + VS section</span>'
            : '<span class="text-muted">Hidden from public</span>'}
        </p>
        ${
          regs.length < 2
...
function render() {
  ...
  // Site nav always visible — Reveal / VS live only inside their own sections
  ...
  if (state.route === "/admin") {
    ...
    // Main page for public visitors = only the published VS poster
    html = renderVsFullscreen(true);
  } else {
    html = renderHome();
  }
...
function renderVsFullscreen(embedded) {
  const regs = state.registrations;
  const m = state.settings.vsMatch || {};
  ...
  // Public view: show ONLY the generated poster image — everything else is hidden
  const pp = (state.publicPoster = state.publicPoster || { key: "", url: "", busy: false, failedKey: "" });
  const ppKey = [a.id, b.id, dateStr, timeStr, (a.logoDataUrl || "").length, (b.logoDataUrl || "").length].join("|");
  if (pp.failedKey !== ppKey) {
    if (pp.key !== ppKey && !pp.busy) {
      pp.busy = true;
      renderVsPoster(a, b, dateStr, timeStr)
        .then((blob) => {
          if (pp.url) URL.revokeObjectURL(pp.url);
          pp.url = URL.createObjectURL(blob);
          pp.key = ppKey;
        })
        .catch(() => { pp.failedKey = ppKey; })
        .finally(() => { pp.busy = false; render(); });
    }
    return `
      <div class="vs-page" style="text-align:center">
        ${
          pp.url
            ? `<img src="${pp.url}" alt="VS poster" style="max-width:100%;width:32rem;height:auto;display:block;margin:0 auto;border-radius:0.75rem" />`
...
function bindEvents() {
  ...
  const uvl = document.getElementById("updateVsLive");
  if (uvl) {
    uvl.addEventListener("click", async () => {
      if (!state.vsA || !state.vsB || state.vsA === state.vsB) {
        state.vsErr = "Select two different teams before updating.";
        render();
        return;
      }
      state.vsErr = "";
...
async function generateVs() {
  if (!state.adminAuthed) return; // only admin can build posters
